...
class NN:
  def __init__(self, NI, NH, NO):
    ...
    self.wi = [[0.09,-0.43],[1.29,0.30],[-0.93,0.86]]
    self.wo = [[1.09],[-1.38]]
    print(self.wi)
    print(self.wo)
  ...
  def runNN (self, inputs):
    ...
    print('INPUT',self.ai)
    print('HIDDEN LAYER',self.ah)
    print('OUTPUT LAYER',self.ao)
    return self.ao
  ...
  def backPropagate (self, targets, N, M):
    ...
    for k in range(self.no):
      ...
      print('ERROR', error,'dsigmoid',dsigmoid(self.ao[k]),'output deltas',output_deltas[k])
  ...
  def train (self, patterns, max_iterations = 2, N=0.5, M=0.1):
